...
| Keyboard | which is to leak the keyboard layout you are using to any website that asks. | Privacy Magic prevents the default behavior of Chrome, |
...
| Time Zone |  | Privacy Magic coalesces time zones so that every time zone offset has a single representative time zone. That way the browser does not leak unnecessary information about your location on the globe. |
| Timing | high resolution timing APIs that allow scripts to characterise your system by measuring tiny differences in the timing of browser behaviors. | Privacy Magic rounds off |
| Touch | whether your device has touch and how many touch points it allows. | Privacy Magic hides from websites |

## Removing tracking breadcrumbs

| Breadcrumb | Standard web browsers | Privacy Magic |
| :--- | :--- | :--- |
| Referrer Policy | Traditionally, browsers will tell each web page you visit, which web page you came from! | Privacy Magic deletes exact page (or path) in the Referer, and only allows the domain to be shared. |
| Tracking Parameters |  |  |
...
Privacy Magic also has remote setting control for specific protections and specific websites. For example, if the Time Zone protection interfered with the proper functioning of Le Monde newspaper website, then we would quickly respond by remotely disabling that particular protection on the lemonde.fr website. Once the protection had been fixed so it no longer breaks the site, then it would be remotely re-enabled.
